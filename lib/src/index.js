const MicroModal = (() => {
  ...
  // Keep references to the modals and what are opened
  const allModals = {}
  const activeModals = []
  ...
  class Modal {
    ...
    showModal (event = null) {
      ...
      activeModals.push(this.modal.id)
    ...
    closeModal (event = null) {
      ...
      // remove reference to active modal
      activeModals.splice(activeModals.indexOf(this.modal.id), 1)
  ...
  const init = config => {
    ...
    for (const key in triggerMap) {
      ...
      allModals[key] = new Modal(options) // eslint-disable-line no-new
    }
  }
  ...
  const show = (targetModal, config) => {
    ...
    const targetId = typeof targetModal === 'string' ? targetModal : targetModal.id
    options.targetModal = targetId

    // Checks if modals and triggers exist in dom
    if (options.debugMode === true && validateModalPresence(targetId) === false) return
  ...
  return { init, show, close, closeAll }
})()
