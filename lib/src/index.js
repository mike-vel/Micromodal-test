const MicroModal = (() => {
  ...
  const validateModalPresence = (modal, debugMode, registered = true) => {
    ...
    if (registered) {
      ...
      modalExists = typeof modal === 'string' ? document.getElementById(modal) !== null : modal // instanceof HTMLElement
    }
    if (!modalExists) {
      if (debugMode) {
        console.warn(
          `MicroModal: \u2757Seems like you have missed %c'${ modal }'%c ID somewhere in your code. Refer example below to resolve it.\n\n` +
  ...
  const validateTriggerPresence = triggers => {
    if (triggers.length <= 0) {
      console.warn(
        ...
        '%cExample:%c <a href="#" data-micromodal-trigger="my-modal"></a>',
        'background-color: #f8f9fa;color: #50596c;font-weight: bold;', '', 'background-color: #f8f9fa;color: #50596c;font-weight: bold;', ''
      )
      return false
    }
  ...
  const initModal = (targetId, config) => {
    ...
    const configs = Object.assign({}, options, config)
    if (validateModalPresence(configs.targetModal, configs.debugMode, false) === false) return
    if (allModals[targetId]) return // already initialized

    allModals[targetId] = new Modal(configs) // eslint-disable-line no-new
  ...
  const show = (targetModal, config) => {
    ...
    if (allModals[targetId]) {
      ...
      allModals[targetId].removeEventListeners() // clear events in case previous modal wasn't closed
    } else {
      initModal(targetId, Object.assign({ targetModal }, config))
    }
  ...
  // Object.assign(exported, { get allModals () { return allModals }, get activeModals () { return activeModals } })
  return exported
})()
