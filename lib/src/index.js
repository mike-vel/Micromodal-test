...
const MicroModal = (() => {
  ...
  options.generateId = () => `micromodal-${++_generatedIdCount}`
  ...
  class Modal {
    constructor ({
      ...
      if (this.modal === null) {
        if (debugMode === true) _validateModalPresence(targetModal, debugMode, false)
        return
      }
    ...
    showModal (event = null) {
      ...
      if (this.modalId && _activeModals.indexOf(this.modalId) === -1) _activeModals.push(this.modalId)
    ...
    closeModal (event = null) {
      ...
      const idx = _activeModals.indexOf(this.modalId)
    ...
    addEventListeners () {
      ...
      if (!this.modalId) {
        document.removeEventListener('keydown', _globalKeydownHandler) // just in case
      } else if (_activeModals.length === 0) {
    ...
    removeEventListeners () {
      ...
      if (_activeModals.length === 0) document.removeEventListener('keydown', _globalKeydownHandler)
    }
     ...
     */
    onClick (event) {
      const overlayClicked = _isOverlay(event.target, this.modal)
      ...
      if (
        ...
        (closestElement !== null && !_isOverlay(closestElement, this.modal) && !overlayClicked) // clicked on a close trigger inside modal
      ) {
  ...
  const _globalKeydownHandler = (event) => {
    const topModal = _allModals[_activeModals[_activeModals.length - 1]]
  ...
  const _isOverlay = (element, modal) => {
   ...
   * @return {boolean}
   */
  const _validateModalPresence = (modal, debugMode, registered = true) => {
    let modalExists
    if (registered) {
      modalExists = _allModals[modal] !== undefined
    } else {
   ...
   * @return {boolean}
   */
  const _validateTriggerPresence = triggers => {
  ...
  const init = config => {
    ...
    if (options.debugMode === true && _validateTriggerPresence(triggers) === false) return
  ...
  const initModal = (targetModal, config = {}) => {
    ...
    const targetId = _getTargetId(targetModal)

    // Checks if a modal exists in dom
    if (!_validateModalPresence(modalConfigs.targetModal, modalConfigs.debugMode, false)) return
    if (_allModals[targetId]) return // already initialized

    _allModals[targetId] = new Modal(modalConfigs) // eslint-disable-line no-new

    // Reference the id on the modal
    _allModals[targetId].modal.setAttribute(options.identifier, targetId)
    _allModals[targetId].modalId = targetId
  }
  ...
  const configModal = (targetModal, config) => {
    const targetId = _getTargetId(targetModal)
    if (_validateModalPresence(targetId, options.debugMode)) Object.assign(_allModals[targetId].config, config)
  }
  ...
  const show = (targetModal, config) => {
    ...
    if (!_allModals[targetId]) {
      initModal(targetId, config)
    } else if (config) {
      configModal(targetModal, config)
    }

    if (_allModals[targetId]) _allModals[targetId].showModal()
  }
  ...
  const close = targetModal => {
    if (_activeModals.length === 0) return // guard against no open modals
    if (targetModal) {
      // Closes the modal that matches the id
      const targetId = _getTargetId(targetModal)

      if (_activeModals.indexOf(targetId) > -1) _allModals[targetId].closeModal()
    } else {
      // Closes the most recent one
      _allModals[_activeModals[_activeModals.length - 1]].closeModal()
    }
  }
   ...
   */
  const closeAll = () => {
    for (let i = _activeModals.length - 1; i >= 0; i--) _allModals[_activeModals[i]].closeModal()
  }
  ...
  const removeModal = (targetModal) => {
    const targetId = _getTargetId(targetModal)
    const modalInstance = _allModals[targetId]
    if (modalInstance) {
      // Hide modal and remove event listeners and triggers
      modalInstance.destroy()

      // Remove from allModals
      delete _allModals[targetId]
    }
  }
