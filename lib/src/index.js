const MicroModal = (() => {
   ...
   * @return {boolean}
   */
  const validateModalPresence = (modal, registered = true) => {
    let modalExists
    if (registered) {
  ...
  const initModal = (targetId, config) => {
    ...
    if (config.debugMode && validateModalPresence(config.targetModal, false) === false) return
  ...
  const show = (targetModal, config) => {
    ...
    if (allModals[targetId]) {
      ...
      initModal(targetId, Object.assign(options, { targetModal }, config))
    }
  ...
  const removeModal = (targetModal) => {
    ...
    if (activeModals.indexOf(targetId) > -1) modalInstance.closeModal()
